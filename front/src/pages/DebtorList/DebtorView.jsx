...
const phoneAddIcon = generateIcon(iconMap.add, null, 'currentColor', 14, 14) // Маленькая иконка для телефона

const DebtorView = () => {
    const {debtId} = useParams()
    const navigate = useNavigate()
    const nodeRef = useRef(null)
    const phoneNodeRef = useRef(null) // Ref для модального вікна телефона
    ...
    // Стан для модального вікна дзвінків
    ...
    const handleSaveCall = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                const errorData = await response.json().catch(() => ({}));
                throw new Error(errorData.message || 'Помилка при збереженні дзвінка');
            }
            ...
            alert('Помилка при збереженні дзвінка: ' + error.message);
        } finally {
